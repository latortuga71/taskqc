...
void* worker_handler(){
    while(1){
        ...
        if (queue_has_item(global_task_queue)){
            ...
            printf("Doing fake work....\n");
            sleep(10);
            printf("Pushing to results\n");
            taskqc_msg* out = taskqc_msg_init(strlen("CCCCC"),"CCCCCC");
...
int main(int argc,char** argv){
    fprintf(stderr,"TODO INIT THREAD POOL FOR CLIENTS\n");
    ...
    int thread_index = 0;
    int error;
    int worker_thread_tid = pthread_create(&thread_pool[thread_index], NULL, &worker_handler, NULL);
    if (worker_thread_tid != 0){
        fprintf(stderr,"error::main::pthread_create::%d",errno);
        return -1;
    }
    pthread_detach(thread_pool[thread_index++]);
    int results_thread_tid = pthread_create(&thread_pool[thread_index], NULL, &results_handler, NULL);
    ...
    while(1){
        ...
        pthread_create(&thread_pool[thread_index++],NULL,&client_handler,&new_fd);
    }
    return 0;
}
